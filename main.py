...
def get_upcoming_birthdays(users):
    upcoming_birthdays=[]
    for user in users:
        birthday_date_srt = user["birthday"].replace(user["birthday"][:4], '2024', 1)
        birthday_date_date = datetime.strptime(birthday_date_srt, "%Y.%m.%d").date()
        ...
        if diff_days <= timedelta(days=6) and diff_days>= timedelta(days=0):
            ...
            birthday_date_srt = birthday_date_date.strftime("%Y-%m-%d")

            to_congratulate = {"name": user["name"], "congratulation_date": birthday_date_srt}
            upcoming_birthdays.append(to_congratulate.copy())

    return(upcoming_birthdays)

users = [
    {"name": "John Doe", "birthday": "1985.01.25"}, {"name": "John Doe", "birthday": "1985.01.27"}
]
